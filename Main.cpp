        ...
        while (FRAME_DURATION <= lag) {
            ...
            if (FRAME_DURATION > lag) {
                ...
                if (1 == field.effect_over()) {
                    if (1 == field.get_game_over()) {
                        draw_text(1, static_cast<unsigned short>(round(0.5f * (CELL_SIZE * COLUMNS - 8 * FONT_WIDTH))), static_cast<unsigned short>(round(0.5f * (CELL_SIZE * ROWS - FONT_HEIGHT))), "VICTORY!", window, sf::Color::White);
                    }
                    else {
                        window.clear();
                        field.restart();
